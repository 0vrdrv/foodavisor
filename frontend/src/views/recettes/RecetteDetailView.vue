<template>
  <div class="px-6 py-4 max-w-4xl mx-auto" v-if="!loading">
    ...
    <div class="flex justify-between items-start mb-4">
      ...
      <!-- Bouton Favori -->
      <button v-if="auth.isAuthenticated()" @click="toggleFavorite(recette.id)" class="text-3xl">
        <span v-if="isFavorite(recette.id)" class="text-yellow-400">★</span>
    ...
    <div class="flex flex-wrap gap-3 mb-10">
      <button class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 rounded text-slate-900 font-medium"
        @click="realiserRecette">
        🍳 Réaliser cette recette
      </button>

      <button v-if="auth.isAdmin() || auth.user?.id === recette.auteur_id"
      ...
      <button v-if="auth.isAdmin() || auth.user?.id === recette.auteur_id"
        class="px-4 py-2 bg-red-600 hover:bg-red-500 rounded" @click="deleteRecette">
        Supprimer
      </button>
    </div>
    ...
    <section class="mb-10">
      ...
      <ul class="space-y-2">
        <li v-for="ing in recette.ingredients" :key="ing.id" class="border border-slate-700 bg-slate-800 p-3 rounded">
    ...
    <section class="mb-10">
      ...
      <ul class="space-y-3">
        <li v-for="et in recette.etapes" :key="et.ord" class="border border-slate-700 bg-slate-800 p-3 rounded">
    ...
    <section class="mb-12">
      ...
      <div v-if="!aDejaRealisee" class="bg-red-600/20 border border-red-600 text-red-400 p-4 rounded-lg mb-6">
        ⚠ Vous devez d'abord <strong>réaliser cette recette</strong> avant de pouvoir laisser un avis.
      </div>

      <!-- Formulaire Avis -->
      <form v-if="aDejaRealisee" @submit.prevent="saveAvis"
        ...
        <div class="flex gap-3">
          ...
          <button v-if="avisExiste" type="button" class="px-4 py-2 bg-red-600 hover:bg-red-500 rounded text-white"
            @click="deleteMyAvis">
            Supprimer mon avis
          </button>
        </div>
    ...
    <section>
      ...
      <ul v-else class="space-y-3">
        <li v-for="av in recette.avis" :key="av.id" class="border border-slate-700 bg-slate-800 p-3 rounded">
          <div class="flex justify-between">
            ...
            <div v-if="av.nom === auth.user?.nom && av.prenom === auth.user?.prenom" class="flex gap-2 text-sm">
              <button class="text-blue-400 hover:text-blue-300" @click="editAvis(av)">
                📝 Modifier
              </button>
              <button class="text-red-400 hover:text-red-300" @click="deleteMyAvis">
                🗑 Supprimer
              </button>
            </div>
          </div>
...
const checkHistorique = async () => {
  try {
    ...
    aDejaRealisee.value = data.some(
      (c) =>
        c.recette_id == route.params.id &&
        c.utilisateur_id == auth.user?.id
    );
  } catch {
...
const loadRecette = async () => {
  try {
    ...
    avis.value = monAvis.value
      ? { note: monAvis.value.note, commentaire: monAvis.value.commentaire }
      : { note: "", commentaire: "" };
...
const editAvis = (avisCourant) => {
  avis.value = {
    note: avisCourant.note,
    commentaire: avisCourant.commentaire,
  };
